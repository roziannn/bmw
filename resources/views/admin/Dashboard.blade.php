...
<head>
    ...
    <style>
        ...
        .modal-content {
            ...
            position: relative;
            max-height: 80%;
            /* Set maximum height for content area */
            overflow-y: auto;
        ...
        .off {
            ...
            /* Merah untuk status off */
        }

        .on {
            background-color: green;
            /* Hijau untuk status on */
        }
    </style>
</head>
...
<div class="container"style="padding-right: 6px;">
            ...
            <form method="POST" id="pengerjaanForm" action="/pengerjaan/000001">
                ...
                <div style="margin-top: 10px">
                    ...
                    <button type="submit" style="font-weight: bold; border-radius: 10px; "
                        class="btn btn-primary">Kerjakan</button>
    ...
    <div col-auto class="mechanic">
        <div class="mechanic-info">
            <img src="https://static.vecteezy.com/system/resources/thumbnails/016/007/776/small_2x/mechanic-creative-icon-design-free-vector.jpg"
                alt="Mekanik 2" width="100" height="100">
            <h2 class="teknisi">{{ $teknisi->isEmpty() ? 'Tidak ada teknisi' : $teknisi->get(0)->nama_teknisi }}</h2>
            <p>Status: <span class="status-icon {{ $teknisi->get(0)->status == 'On Working' ? 'off' : 'on' }}"></span>
                <br>{{ $teknisi->get(0)->status }}
            </p>
            @if (!$teknisi->isEmpty() && $teknisi->get(0)->status == 'On Working')
                ...
                <button id="serviceButton"
                    style="border-radius: 10px; font-size: 10px; background-color: green;margin-top: 20px;"
                    class="btn btn-primary" onclick="performService('{{ $teknisi->get(0)->id_teknisi }}')">Service
                    Done.</button>
                <div id="serviceButton"
                    style="display: none; background-color: #dff0d8; border: 1px solid #d0e9c6; padding: 10px; border-radius: 5px; margin-top: 10px;">
                </div>
            @endif
        ...
        <div class="mechanic-info" onClick()>
            ...
            <p>Status: <span class="status-icon {{ $teknisi->get(1)->status == 'On Working' ? 'off' : 'on' }}"></span>
                <br>{{ $teknisi->get(1)->status }}
            </p>
            @if (!$teknisi->isEmpty() && $teknisi->get(1)->status == 'On Working')
                ...
                <button id="serviceButton"
                    style="border-radius: 10px; font-size: 10px; background-color: green;margin-top: 20px;"
                    class="btn btn-primary" onclick="performService('{{ $teknisi->get(1)->id_teknisi }}')">Service
                    Done.</button>
            @endif
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="modal-body">
                                    ...
                                    <div class="mb-1">
                                        <label for="listTechnicians" class="form-label"
                                            style="color: black; font-weight: bold;">Pilih Teknisi:</label>
                                        <div class="d-flex">
                                            @foreach ($teknisiAvailable as $teknisiList)
                                                <div class="mr-3">
                                                    <input type="radio"
                                                        id="technicianInput{{ $teknisiList->id_teknisi }}"
                                                        name="technician" value="{{ $teknisiList->id_teknisi }}">
                                                    <label
                                                        for="technicianInput{{ $teknisiList->id_teknisi }}">{{ $teknisiList->nama_teknisi }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
        ...
        <div class="mechanic-info">
            ...
            <p>Status: <span class="status-icon {{ $teknisi->get(2)->status == 'On Working' ? 'off' : 'on' }}"></span>
                <br>{{ $teknisi->get(2)->status }}
            </p>
            @if (!$teknisi->isEmpty() && $teknisi->get(2)->status == 'On Working')
                <button class="detail-button" data-teknisi-id="{{ $teknisi->get(2)->id_teknisi }}">See
                    Detail</button>
                <button id="serviceButton"
                    style="border-radius: 10px; font-size: 10px; background-color: green;margin-top: 20px;"
                    class="btn btn-primary" onclick="performService('{{ $teknisi->get(2)->id_teknisi }}')">Service
                    Done.</button>
            @endif
        ...
        <div class="mechanic-info">
            ...
            <p>Status: <span class="status-icon {{ $teknisi->get(3)->status == 'On Working' ? 'off' : 'on' }}"></span>
                <br>{{ $teknisi->get(3)->status }}
            </p>
            @if (!$teknisi->isEmpty() && $teknisi->get(3)->status == 'On Working')
                <button class="detail-button" data-teknisi-id="{{ $teknisi->get(3)->id_teknisi }}">See
                    Detail</button>
                <button id="serviceButton"
                    style="border-radius: 10px; font-size: 10px; background-color: green;margin-top: 20px;"
                    class="btn btn-primary" onclick="performService('{{ $teknisi->get(3)->id_teknisi }}')">Service
                    Done.</button>
            @endif
        ...
        <div class="mechanic-info">
            ...
            <p>Status: <span class="status-icon {{ $teknisi->get(4)->status == 'On Working' ? 'off' : 'on' }}"></span>
                <br>{{ $teknisi->get(4)->status }}
            </p>
            @if (!$teknisi->isEmpty() && $teknisi->get(4)->status == 'On Working')
                <button class="detail-button" data-teknisi-id="{{ $teknisi->get(4)->id_teknisi }}">See
                    Detail</button>
                <button id="serviceButton"
                    style="border-radius: 10px; font-size: 10px; background-color: green;margin-top: 20px;"
                    class="btn btn-primary" onclick="performService('{{ $teknisi->get(4)->id_teknisi }}')">Service
                    Done.</button>
            @endif
                    ...
                    fetch(`/detail/task/${selectedValue}`)
                        ...
                        .then(data => {
                            ...
                            data.Layanan.forEach(layanan => {
                                ...
                                maintenanceForm.appendChild(document.createElement(
                                    'br'));
                            });
                            const radioButtons = maintenanceForm.querySelectorAll(
                                '.maintenance-option');
    ...
    <script>
        function performService(teknisiId) {
            // Panggil API atau endpoint untuk mengubah status teknisi
            fetch(`/updateDone/${teknisiId}`, {
                ...
                .then(response => response.json())
                .then(data => {
                    // Tampilkan notifikasi dan ubah tampilan tombol jika perubahan berhasil
                    var notificationElement = document.getElementById('serviceButton');
                    notificationElement.innerHTML = 'Service selesai dapat melanjutkan pembayaran.';
                    notificationElement.style.display = 'block';
                    alert('Service selesai. Silakan melanjutkan pembayaran.');
                    ...
                    button.innerHTML = "Service Completed";

                    var statusElement = document.querySelector(`[data-teknisi-id="${teknisiId}"]`);
                    if (statusElement) {
    ...
    <script>
        //script buat detail teknisi mengerjakan apa
            ...
            fetch(`/teknisi/mengerjakan/${teknisiId}`)
                ...
                .then(data => {
                    ...
                    allLayanan.forEach(layanan => {
                        ...
                        label.appendChild(input);
                        label.appendChild(document.createTextNode(` ${layanan}`));
